...
class SAGE(torch.nn.Module):
    def __init__(self,num_features,num_classes, hidden_layer = 16, dropout = 0.5):
        super().__init__()
        self.conv1 = SAGEConv(num_features, hidden_layer)
        self.conv2 = SAGEConv(hidden_layer, num_classes)
...
class Model_SAGE(TargetModelInterface):
    ...
    def __init__(self, data, model_name, dataset_name, hidden_size = 16,epochs = 200, lr = 0.01, weight_decay=5e-4, device = 'cpu', dropout = 0.5, private_parameters = None, save_path = ""):
        ...
        self.wrapped_model = SAGE(num_classes=len(torch.unique(self.data.y)), num_features=self.data.num_features, hidden_layer=hidden_size)
